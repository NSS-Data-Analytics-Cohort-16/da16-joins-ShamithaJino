...
SELECT company_name, count(*) AS no_of_movies
...
SELECT film_title,company_name,imdb_rating, COUNT(s.movie_id)
FROM distributors AS d
LEFT JOIN specs AS s
ON d.distributor_id = s.domestic_distributor_id
...
GROUP BY film_title,company_name,imdb_rating
ORDER BY imdb_rating DESC;
...
INNER JOIN rating AS r
ON s.movie_id = r.movie_id
INNER JOIN revenue re
ON re.movie_id= r.movie_id
GROUP BY decade

--1.b) Then alter your query so it returns JUST the second highest average imdb rating and its decade. 
